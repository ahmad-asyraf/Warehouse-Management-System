<%-- 
    Document   : details
    Created on : 13 May 2022, 12:27:33 am
    Author     : Asyraf
--%>

<%
    String userSession = (String)session.getAttribute("username");
    String userType1 = "ADMIN";
    String userType2 = "STAFF";
    String userType3 = "USER";
    int i = 1, result = 0;

    if(userSession == null) {
        response.sendRedirect("../index.html");
    }
    if(userSession != null && userType1 == "ADMIN") {
        connectionDao con = new connectionDao();

        Statement stmt = con.getConnection().createStatement();
        String sql = "SELECT * FROM users WHERE email='"+userSession+"'AND userType='" + userType1 + "'";
        
        ResultSet rs = stmt.executeQuery(sql);

        while(rs.next()) 
        {
%>
<%@page import="java.security.MessageDigest"%>
<%@page import="java.sql.*"%>
<%@page import="model.connectionDao"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <title>ADMIN - Users List</title>
        <meta name="author" content="Asyraf S60494">
        <link rel="stylesheet" href="../css/main-style.css"/>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <link href="../css/bootstrap.min.css" rel="stylesheet">
    </head>
    <body>
        <div class="container-fluid py-5">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="navbar1">
                            <div class="push-left">
                                <a href="main.jsp" class=""><span class="bi bi-house"></span> DASHBOARD</a>
                                <a href="profile.jsp" class=""><span class="bi bi-person-circle"></span> PROFILE</a>
                                <!-- <a href="" class=""><span class="bi bi-envelope"></span> MESSAGES</a> -->
                                <a href="registerProduct.jsp" class=""><span class="bi bi-box-arrow-in-right"></span> REGISTER ITEM</a>
                                <a href="stats.jsp" class=""><span class="bi bi-bar-chart-line"></span> STAT</a>
                                <a href="list.jsp" class="active"><span class="bi bi-people-fill"></span> USERS LIST</a>
                            </div>
                            <div class="push-right">
                                <a href="../controller/logout.jsp" class=""><span class="bi-box-arrow-left"></span> LOGOUT</a>
                            </div>
                        </div>
                    </div>
                </div>
                
                <div class="row py-5">
                    <div class="col-4">
                        <div class="profile-box">
                            <div class="">
                                <img src="../images/male-icon.jpg" class="img-fluid" alt="alt"/>
                            </div>
                            <div class="mt-4">
                                <p class="text-center">
                                    <%= rs.getString(4) + " " + rs.getString(5) %>
                                </p>
                                <p class="form-text text-center"><%= rs.getString(9) %></p>
                            </div>
                        </div>
                    </div>
                    <div class="col-8">
                        <div class="content-box">
                            <div class="container">
                                <div class="row">
                                    <div class="col-12">
                                        <a class="form-label" href="list.jsp">&lt; back</a>
                                    </div>
                                    <div class="col-12">
                                        <%
                                            String usersID = "";
                                            if(request.getParameter("usersID") != null) {
                                                usersID = request.getParameter("usersID");
                                            }
                                            
                                            Statement stmt1 = con.getConnection().createStatement();
                                            String sql1 = "SELECT * FROM users WHERE userid='" + usersID + "'";
                                            ResultSet rs1 = stmt1.executeQuery(sql1);
                                            
                                            if(rs1 != null && rs1.next()) 
                                            {
                                          %>
                                            <table class="table table-striped">
                                                <thead>
                                                    <tr>
                                                        <th>User Details</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <th>User ID</th>
                                                        <td><%= rs1.getString(1) %></td>
                                                    </tr>
                                                    <tr>
                                                        <th>Email</th>
                                                        <td><%= rs1.getString(2) %></td>
                                                    </tr>
                                                    <tr>
                                                        <th>First Name</th>
                                                        <td><%= rs1.getString(4) %></td>
                                                    </tr>
                                                    <tr>
                                                        <th>Last Name</th>
                                                        <td><%= rs1.getString(5) %></td>
                                                    </tr>
                                                    <tr>
                                                        <th>Address</th>
                                                        <td><%= rs1.getString(6) %></td>
                                                    </tr>
                                                    <tr>
                                                        <th>Mobile No</th>
                                                        <td><%= rs1.getString(7) %></td>
                                                    </tr>
                                                    <tr>
                                                        <th>User Type</th>
                                                        <td><%= rs1.getString(9) %></td>
                                                    </tr>
                                                    <tr>
                                                        <th>Account Creation</th>
                                                        <td><%= rs1.getString(10) %></td>
                                                    </tr>
                                                </tbody>
                                          </table>
                                        <%  }%>
                                    </div>
                                    <div class="col-12 mt-5">
                                        <form action="../controller/grantAccess.jsp" method="POST" onsubmit="return confirm('Confirm to grant access?')">
                                        <h2>Grant Access</h2>
                                        <div class="row mt-3">
                                            <div class="col-2">
                                                User Type
                                            </div>
                                            <div class="col-4">
                                                <input type="hidden" name="usersID" value="<%= rs1.getString(1) %>">
                                                <select name="grantUser" class="form-control">
                                                    <option>STAFF</option>
                                                    <option>USER</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row mt-3">
                                            <div class="offset-2 col-4">
                                                <button class="btn btn-warning" type="submit">Confirm?</button>
                                            </div>
                                        </div>
                                        </form>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        
        <script src="../js/bootstrap.bundle.min.js"></script>
    </body>
<%      
        }
    }
    if(userSession != null && userType2 == "STAFF") {
        connectionDao con = new connectionDao();

        Statement stmt = con.getConnection().createStatement();
        String sql = "SELECT * FROM users WHERE email='"+userSession+"'AND userType='" + userType2 + "'";
        
        ResultSet rs = stmt.executeQuery(sql);

        while(rs.next()) 
        {
%>
    <head>
        <title>Staff - Stock List</title>
        <meta name="author" content="Asyraf S60494">
        <link rel="stylesheet" href="../css/main-style.css"/>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <link href="../css/bootstrap.min.css" rel="stylesheet">
    </head>
    <body>
        <div class="container-fluid py-5">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="navbar1">
                            <div class="push-left">
                                <a href="main.jsp" class=""><span class="bi bi-house"></span> DASHBOARD</a>
                                <a href="profile.jsp" class=""><span class="bi bi-person-circle"></span> PROFILE</a>
                                <!-- <a href="" class=""><span class="bi bi-envelope"></span> MESSAGES</a> -->
                                <a href="registerProduct.jsp" class=""><span class="bi bi-box-arrow-in-right"></span> PRODUCT</a>
                                <!-- <a href="" class=""><span class="bi bi-bar-chart-line"></span> STAT</a> -->
                                <a href="list.jsp" class="active"><span class="bi bi-clipboard-data"></span> STOCK LIST</a>
                            </div>
                            <div class="push-right">
                                <a href="../controller/logout.jsp" class=""><span class="bi-box-arrow-left"></span> LOGOUT</a>
                            </div>
                        </div>
                    </div>
                </div>
                
                <div class="row py-5">
                    <div class="col-4">
                        <div class="profile-box">
                            <div class="">
                                <img src="../images/male-icon.jpg" class="img-fluid" alt="alt"/>
                            </div>
                            <div class="mt-4">
                                <p class="text-center">
                                    <%= rs.getString(4) + " " + rs.getString(5) %>
                                </p>
                                <p class="form-text text-center"><%= rs.getString(9) %></p>
                            </div>
                        </div>
                    </div>
                    <div class="col-8">
                        <div class="content-box">
                            <div class="container">
                                <div class="row">
                                    <div class="col-12">
                                        <%
                                            String ProductCode = "";
                                            if(request.getParameter("ProductCode") != null) {
                                                ProductCode = request.getParameter("ProductCode");
                                            }
                                            
                                            Statement stmt1 = con.getConnection().createStatement();
                                            String sql1 = "SELECT * FROM product WHERE prodID='" + ProductCode + "'";
                                            ResultSet rs1 = stmt1.executeQuery(sql1);
                                            
                                            if(rs1 != null && rs1.next()) 
                                            {
                                          %>
                                            <table class="table">
                                                <tr>
                                                    <td colspan="2"><img src="../images/warehouse-logo.jpg" class="img-fluid img-thumbnail" alt="alt"/></td>
                                                </tr>
                                                <tr>
                                                    <th>Product code</th>
                                                    <td><%= rs1.getString(1) %></td>
                                                </tr>
                                                <tr>
                                                    <th>Product name</th>
                                                    <td><%= rs1.getString(2) %></td>
                                                </tr>
                                                <tr>
                                                    <th>Product description</th>
                                                    <td><%= rs1.getString(3) %></td>
                                                </tr>
                                                <tr>
                                                    <th>Quantity</th>
                                                    <td><%= rs1.getString(4) %></td>
                                                </tr>
                                                <tr>
                                                    <th>Price/tray</th>
                                                    <td><%= rs1.getString(5) %></td>
                                                </tr>
                                                <tr>
                                                    <th>Status</th>
                                                    <td><%= rs1.getString(6) %></td>
                                                </tr>
                                          </table>
                                        <%  }%>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        
        <script src="../js/bootstrap.bundle.min.js"></script>
    </body>
</html>
<%
        }
    }
%>